...
public class DataStreamAsDisjointIntervals {
    ...
    public void addNum(int val) {
        if (containsVal(getIntervals(), val))
            return;
    ...
    private static boolean containsVal(List<Interval> list, int val) {

        return 0 <= Collections.binarySearch(list, new Interval(val, val), new Comparator<Interval>() {
            @Override
            public int compare(Interval o1, Interval o2) {
                if (o2.start < o1.start)
                    return 1;
                else if (o2.start <= o1.end)
                    return 0;
                else
                    return -1;
            }
    ...
    public static void main(String[] args) {
        ...
        System.out.println(DataStreamAsDisjointIntervals.containsVal(Arrays.asList(
                new Interval(4,4), new Interval(6,8)
        ), 3));
    }
}
