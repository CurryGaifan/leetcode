...
public class BestTimetoBuyandSellStockwithCooldown {
    ...
    private int maxProfit(int[] prices, int index) {
        ...
        int sellIndex = findNextSell(prices, index);
        int curProfit = prices[sellIndex] - prices[index];
        int max = curProfit + maxProfit(prices, sellIndex + 2);

        if (sellIndex - 1 >= index) {
            curProfit = prices[sellIndex - 1] - prices[index];
        } else {
            curProfit = 0;
        }

        int maxProfit = maxProfit(prices, sellIndex + 1);

        max = Math.max(max, curProfit + maxProfit);
        if (sellIndex + 1 < prices.length) {
            max = Math.max(max, prices[sellIndex + 1] - prices[index] + maxProfit);
        }
...
//        System.out.println(index + " " + max);
    ...
    private int findNextSell(int[] prices, int start) {
